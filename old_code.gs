function lcsActuaold() {
  ...
  for (let i = 0; i < sprintStartDateold.length; i++) {
    console.log(sprintStartDateold[i]);

    for (let j = 0; j < 1; j++) {
      console.log(lcs[rowStartIndexold[i] - 1 + j]);
      var url =
        "https://analytics.api.aiesec.org/v2/applications/analyze.json?access_token=" +
        "&start_date=" +
        sprintStartDateold[i] +
        "&end_date=" +
        sprintEndDateold[i] +
        "&performance_v3%5Boffice_id%5D=" +
        lcs_codesold[lcs[rowStartIndexold[i] - 1 + j]];
      ...
      sheet
        .getRange(rowStartIndexold[i] + j, 2, 1, list[0].length)
        .setValues(list);
    }
  }
}
